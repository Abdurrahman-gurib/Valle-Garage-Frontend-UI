...
export default function VehicleOut(){
  ...
  async function markIn(row){ await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3000/api'}/vehicle-out/${row.id}`, {method:'PATCH', headers:{'Content-Type':'application/json', Authorization:`Bearer ${localStorage.getItem('valle-token')||localStorage.getItem('token')||''}`}, body:JSON.stringify({recordIn:true})}); await refreshAll(); setMsg(`Vehicle IN recorded for ${row.vehicle}. Exact total time is updated.`); }
