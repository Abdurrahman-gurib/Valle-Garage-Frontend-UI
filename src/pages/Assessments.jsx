...
export default function Assessments(){
 ...
 {modal?.type==='view' && <Modal title={`${modal.item.id} Assessment Details`} onClose={()=>setModal(null)} wide><AssessmentDetail assessment={modal.item} role={currentUser.role} inventory={inventory} onIssue={()=>{issuePartsForAssessment(modal.item.id); setModal(null)}} onComplete={()=>{completeAssessment(modal.item.id); setModal(null)}} onReopen={(reason)=>{reopenAssessment(modal.item.id, reason); setModal(null)}} onUpdate={(updates)=>{updateAssessment(modal.item.id,updates); setModal(null)}} /></Modal>}
 </div>
}
...
function AssessmentDetail({assessment, role, inventory, onComplete, onIssue, onReopen, onUpdate}){
 ...
 <div className="button-row"><Button onClick={()=>onUpdate({issue, conclusion, parts})}>Save Updates</Button>{role==='store' && assessment.status!=='Completed' && <Button variant="warning" onClick={onIssue}>Issue Parts & Complete</Button>}{role!=='store' && assessment.status!=='Completed' && <Button variant="secondary" onClick={onComplete}>Mark Complete</Button>}</div>
