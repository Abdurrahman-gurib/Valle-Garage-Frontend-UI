...
export default function Wheel(){
  const { currentUser, vehicles, wheelRequests=[], wheelStock=[], addWheelRequest, updateWheelRequest, updateWheelStock, nowLocalInput, refreshAll } = useApp();
  const [q,setQ]=useState('');
  const [stockSearch,setStockSearch]=useState('');
  const [period,setPeriod]=useState('today');
  const [custom,setCustom]=useState({from:'',to:''});
  const [editStock,setEditStock]=useState(null);
  const [stockDraft,setStockDraft]=useState(DEFAULT_WHEELS);
  const [form,setForm]=useState({ vehicleId:'', reason:'', wheels:[{wheelType:'Quad Front Wheel', quantity:1}] });
  const [msg,setMsg]=useState('');
  useEffect(()=>{ if(wheelStock.length) setStockDraft(wheelStock); },[wheelStock]);
  ...
  function saveStock(next){ setStockDraft(next); }
  async function persistStock(row){ const saved = await updateWheelStock?.(row.id, row); if(saved){ setMsg(`Wheel stock saved for ${saved.wheelType}.`); setEditStock(null); } }
  ...
  async function issue(w){ await updateWheelRequest(w.id,{status:'ISSUED', issuedAt:nowLocalInput?.() || new Date().toISOString()}); await refreshAll?.(); }
