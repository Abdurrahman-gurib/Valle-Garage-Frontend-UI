  ...
  return <div className="page reports-page">
    ...
    <div className="analytics-grid">
      ...
      {(isAdmin||isStore) && <>
        ...
        <ChartCard title="Stock Risk Overview" subtitle="Live DB count by stock risk level." onExport={()=>exportChartXlsx('stock-risk-overview', stockRiskData)}><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={stockRiskData} dataKey="value" nameKey="label" innerRadius={58} outerRadius={92} label={({label,value})=>`${label}: ${value}`}>{stockRiskData.map((_,i)=><Cell key={i} fill={COLORS[i%COLORS.length]}/>)}</Pie><Tooltip/><Legend/></PieChart></ResponsiveContainer></ChartCard>
        <ChartCard title="Parts by Category" subtitle="Category distribution from InventoryItem table." onExport={()=>exportChartXlsx('parts-by-category', categoryChartData)}><ResponsiveContainer width="100%" height="100%"><BarChart data={categoryChartData} layout="vertical" margin={{left:80,right:24,top:8,bottom:8}}><CartesianGrid strokeDasharray="3 3"/><XAxis type="number" allowDecimals={false}/><YAxis type="category" dataKey="label" width={120} tick={{fontSize:11}}/><Tooltip content={<ChartTooltip valueLabel="Parts" xAxisName="Category" yAxisName="Parts Count"/>}/><Legend/><Bar dataKey="value" name="Parts count" fill="#6f3cff" radius={[0,8,8,0]}/></BarChart></ResponsiveContainer></ChartCard>
        ...
        <ChartCard title="Stock Value by Category" subtitle="Inventory stock value calculated from current stock × cost price." onExport={()=>exportChartXlsx('stock-value-by-category', stockValueByCategoryData)}>{stockValueByCategoryData.length ? <ResponsiveContainer width="100%" height="100%"><BarChart data={stockValueByCategoryData} layout="vertical" margin={{left:80,right:24,top:8,bottom:8}}><CartesianGrid strokeDasharray="3 3"/><XAxis type="number"/><YAxis type="category" dataKey="label" width={120} tick={{fontSize:11}}/><Tooltip content={<ChartTooltip valueLabel="Stock Value" xAxisName="Category" yAxisName="Value (MUR)"/>}/><Legend/><Bar dataKey="value" name="Stock value" fill="#24f66f" radius={[0,8,8,0]}/></BarChart></ResponsiveContainer> : <Empty/>}</ChartCard>
      </>}
    </div>
