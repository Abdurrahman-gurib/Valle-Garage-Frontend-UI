...
import {
  ...
  Scatter, ScatterChart, ResponsiveContainer, Tooltip, XAxis, YAxis, Legend
} from 'recharts';
...
async function exportProfessionalXlsx({ fileName, title, reportType, filters, headers, rows, summaryRows = [], currentUser = {}, sourcePage = 'Executive Reports & System Tracking' }){
  ...
  const analysisPromptRows = [
    ['VALLÉ GARAGE OPERATIONS - ANALYSIS PROMPT'],
    [],
    ['Purpose'],
    ['Analyze the live database extract in the Valle Report worksheet and provide accurate, useful insights based only on the visible filtered data.'],
    [],
    ['Prompt to use'],
    ...
    [],
    ['Report Type', readableReportType],
    ['Selected Period', periodLabel(filters)],
    ['Rows Exported', rows?.length || 0],
    ['Generated At', generatedAt],
    [],
    ['Suggested Insight Checks'],
    ['1. Verify the row count and selected filter period before drawing conclusions.'],
    ['2. Highlight top vehicles, tickets, mechanics, parts, fuel usage, time out, and low stock based on this report type.'],
    ['3. Identify outliers, missing values, zero-cost rows, delayed vehicle returns, repeated parts, or stock risks.'],
    ['4. Add simple charts where useful: trend by date/time, count by vehicle plate, quantity by part, fuel by vehicle, or duration by vehicle.'],
    ['5. Clearly separate operational insights from financial insights depending on the user role and report type.'],
  ];
  const analysisSheet = XLSX.utils.aoa_to_sheet(analysisPromptRows);
  analysisSheet['!cols'] = [{ wch: 26 }, { wch: 120 }];
  XLSX.utils.book_append_sheet(workbook, analysisSheet, 'Analysis Prompt');
...
function exactOrContainsPlate(value, query, allPlates=[]){ const q=normPlate(query); if(!q) return true; const exactExists=(allPlates||[]).some(p=>normPlate(p)===q); const p=normPlate(value); return exactExists ? p===q : p.includes(q); }
function plate(v){
...
function shortInventoryLabel(value){ const text=String(value||'-').trim(); return text.length<=18 ? text : `${text.slice(0,8)}…${text.slice(-7)}`; }
function cleanInventoryItem(item){ return { ...item, sku:item?.sku||'', name:item?.name||item?.part||'', category:item?.category||'Uncategorised', barcode:item?.barcode||'', stock:n(item?.stock ?? item?.currentStock ?? 0), reorderLevel:n(item?.reorderLevel ?? 0), costPrice:n(item?.costPrice ?? 0), sellingPrice:n(item?.sellingPrice ?? item?.lastPrice ?? item?.price ?? 0), location:item?.location||'', supplierName:item?.supplierName||item?.supplier||'', supplierEmail:item?.supplierEmail||'' }; }
function LowStockTooltip({active,payload,label}){ if(!active||!payload?.length) return null; const row=payload[0]?.payload||{}; return <div className="analytics-tooltip"><b>{row.fullLabel||label}</b><span>Current stock: {row.stock}</span><span>Reorder level: {row.reorder}</span><span>Part: {row.name||'-'}</span><span>Location: {row.location||'-'}</span></div>; }
...
function StockDetailPanel({ rows, title, subtitle, onExport }){
  const list = rows || [];
  return <ChartCard title={title} subtitle={subtitle} onExport={onExport}>
    {list.length ? <div className="stock-report-fit-list">
      <div className="stock-report-fit-head"><span>SKU</span><span>Part</span><span>Stock</span><span>Reorder</span><span>Location</span></div>
      {list.map((item, idx)=><div className="stock-report-fit-row" key={`${item.sku || item.name}-${idx}`}>
        <span title={item.sku}>{item.sku || '-'}</span>
        <span title={item.name}><b>{item.name || '-'}</b><small>{item.category || '-'}</small></span>
        <span className={item.stock <= 0 ? 'stock-zero-pill' : 'stock-low-pill'}>{item.stock}</span>
...
export default function Reports(){
  const { currentUser, vehicles, inventory, assessments, garageOps, fuelConsumptions, vehicleOutActivities, externalPartsIssues = [], wheelRequests = [], fuelStockLogs = [], refreshAll } = useApp();
  ...
  const data=useMemo(()=>{
    ...
    const allPlates = vehicles.map(v=>v.plate).filter(Boolean);
    const fuel=fuelConsumptions.filter(f=>periodOk(f.recordedAt,filters)&&exactOrContainsPlate(f.vehicle, plateQ, allPlates));
    const externalParts=externalPartsIssues.filter(p=>periodOk(p.issuedAt || p.createdAt, filters));
    const out=vehicleOutActivities.filter(o=>periodOk(o.startDateTime,filters)&&exactOrContainsPlate(o.vehicle, plateQ, allPlates));
    const ass=assessments.filter(a=>periodOk(a.createdAt,filters)&&exactOrContainsPlate(a.vehicle, plateQ, allPlates));
    const ops=garageOps.filter(g=>periodOk(g.start||g.createdAt,filters)&&exactOrContainsPlate(g.vehicle, plateQ, allPlates));
    const issuedParts=ass.flatMap(a=>(a.parts||[]).map(p=>({...p, vehicle:a.vehicle, ticket:a.id, date:a.createdAt, source:'Mechanic assessment issue'})));
    const distributionParts=externalParts.map(p=>({...p, name:p.partName || p.name, sku:p.sku, qty:p.quantity || p.qty || 1, vehicle:p.receiverName || p.receiverCompany || p.receiverType || 'Parts Distribution', ticket:p.referenceNo || p.id, date:p.issuedAt || p.createdAt, source:'Parts distribution / sell'}));
    const allStoreParts=[...issuedParts, ...distributionParts];
    const usedParts=[];
    const partsForCost=allStoreParts;
    const wheel = (wheelRequests || []).filter(w=>periodOk(w.issuedAt || w.createdAt, filters));
    const fuelStock = (fuelStockLogs || []).filter(l=>periodOk(l.deliveredAt || l.createdAt || l.recordedAt, filters));
    return { fuel,out,ass,ops,issuedParts,distributionParts,allStoreParts,usedParts,partsForCost,wheel,fuelStock };
  },[fuelConsumptions,vehicleOutActivities,externalPartsIssues,wheelRequests,fuelStockLogs,assessments,garageOps,vehicles,filters,selected]);
  ...
  const analytics=useMemo(()=>({
    ...
    fuelByVehicleType: group(data.fuel,f=>{ const v=vehicles.find(x=>normPlate(x.plate)===normPlate(f.vehicle)||x.id===f.vehicleId||x.dbId===f.vehicleId); return (v?.vehicleType || v?.type || v?.model || 'Unknown').toUpperCase(); },f=>n(f.fuelLitres)).slice(0,12),
    petrolByVehicleType: group(data.fuel.filter(f=>String(f.fuelType||'').toUpperCase().includes('PETROL')),f=>{ const v=vehicles.find(x=>normPlate(x.plate)===normPlate(f.vehicle)||x.id===f.vehicleId||x.dbId===f.vehicleId); return (v?.vehicleType || v?.type || v?.model || 'Unknown').toUpperCase(); },f=>n(f.fuelLitres)).slice(0,12),
    dieselByVehicleType: group(data.fuel.filter(f=>String(f.fuelType||'').toUpperCase().includes('DIESEL')),f=>{ const v=vehicles.find(x=>normPlate(x.plate)===normPlate(f.vehicle)||x.id===f.vehicleId||x.dbId===f.vehicleId); return (v?.vehicleType || v?.type || v?.model || 'Unknown').toUpperCase(); },f=>n(f.fuelLitres)).slice(0,12),
    ...
    outDuration: group(data.out,o=>o.vehicle,o=>n(o.durationSeconds) || secs(o.startDateTime,o.endDateTime)).slice(0,15).map(row=>({ ...row, displayValue:dur(row.value), durationLabel:dur(row.value), subLabel:'Actual OUT→IN time in days/hours/minutes/seconds' })),
    ...
    partsIssued: group(data.allStoreParts,p=>p.name || p.sku || 'Part',p=>n(p.qty||p.quantity||1)).slice(0,10),
    partsByDestination: group(data.allStoreParts,p=>p.source || p.receiverType || 'Mechanic issue',p=>n(p.qty||p.quantity||1)).slice(0,10),
    wheelByType: group(data.wheel,p=>p.wheelType || 'Wheel',p=>n(p.quantity||1)).slice(0,12),
    wheelByVehicleType: group(data.wheel,p=>p.vehicleType || 'Unknown',p=>n(p.quantity||1)).slice(0,12),
    fuelRefillByType: group(data.fuelStock,p=>p.fuelType || 'Fuel refill',p=>n(p.litres||p.fuelLitres||p.quantity)).slice(0,12),
    mechanicAssignments: data.ass.flatMap(a => String(a.mechanic || 'Unassigned').split(',').map(m => ({ label:`${m.trim() || 'Unassigned'} • ${a.id} • ${a.vehicle}`, value:1, mechanic:m.trim() || 'Unassigned', ticket:a.id, plate:a.vehicle, status:a.status }))).slice(0,50),
    partsUsed: [],
  ...
  const stockValueByCategoryData = useMemo(() => group(inventoryClean, (item)=>item.category || 'Uncategorised', (item)=>n(item.stock) * n(item.costPrice)).filter(x=>x.value>0).slice(0,8), [inventoryClean]);
  const lowStockChartData = useMemo(() => inventoryLowStock.map((item)=>({ label:shortInventoryLabel(item.sku||item.name||'-'), fullLabel:`${item.sku||'-'} - ${item.name||''}`, name:item.name, sku:item.sku, category:item.category, stock:item.stock, reorder:item.reorderLevel, location:item.location, value:item.stock })).sort((a,b)=>(a.stock-b.stock)||(b.reorder-a.reorder)), [inventoryLowStock]);
  const zeroStockChartData = useMemo(() => inventoryClean.filter((item)=>item.stock<=0).map((item)=>({ label:shortInventoryLabel(item.sku||item.name||'-'), fullLabel:`${item.sku||'-'} - ${item.name||''}`, name:item.name, sku:item.sku, category:item.category, stock:item.stock, reorder:item.reorderLevel, location:item.location, value:item.reorderLevel || 1 })).sort((a,b)=>(b.reorder-a.reorder)||String(a.label).localeCompare(String(b.label))), [inventoryClean]);
  const allowedReports = isFuel ? ['fuel'] : isVehicle ? ['vehicle-out'] : isStore ? ['parts','wheel','low-stock','garage','vehicle-cost','cost','all'] : ['all','fuel','vehicle-out','garage','vehicle-cost','parts','wheel','cost','mechanics','low-stock'];
  const effectiveReport = allowedReports.includes(filters.reportType) ? filters.reportType : allowedReports[0];

  function reportHeaders(){
    if(effectiveReport==='fuel') return ['Recorded Date/Time','Vehicle Plate','Fuel Type','Quantity (L)','Meter Reading','Recorded By','Notes'];
    if(effectiveReport==='vehicle-out') return ['Out Date/Time','In Date/Time','Vehicle Plate','Invoice','Guide','Activity','Actual Total Time Out (D/H/M/S)','Destination','Notes'];
    ...
    if(effectiveReport==='wheel') return ['Stored Date/Time','Date','Time','Month','Year','Vehicle Plate','Vehicle Type','Wheel Type','Quantity','Reason','Status','Issued At','Issued By'];
    if(effectiveReport==='low-stock') return ['Extracted Date/Time','SKU','Part','Stock','Reorder Level','Supplier','Category','Location'];
    if(effectiveReport==='mechanics') return ['Stored Date/Time','Date','Time','Month','Year','Vehicle Plate','Ticket','Status','Assigned Mechanic','Issue / Work','Hours','Notes'];
  ...
  const tableRows = useMemo(()=>{
    if(effectiveReport==='fuel') return data.fuel.map(f=>{
      const meter = [f.meterReading, f.meterType].filter(Boolean).join(' ');
      return [formatFullDateTime(f.recordedAt), f.vehicle, f.fuelType, `${n(f.fuelLitres).toLocaleString()} L`, meter || '-', f.recordedBy || '-', f.notes || ''];
    });
    if(effectiveReport==='vehicle-out') return data.out.map(o=>[
      formatFullDateTime(o.startDateTime),
      o.endDateTime ? formatFullDateTime(o.endDateTime) : 'Currently Out',
      o.vehicle,
      o.invoiceNumber || '-',
      o.guideName || '-',
      o.bikingVehicleType || o.quadActivity || o.activityType || '-',
      dur(secs(o.startDateTime,o.endDateTime)),
      o.destinationRoute || o.destination || '-',
      o.notes || '-'
    ]);
    ...
    if(effectiveReport==='wheel') return data.wheel.map(w=>{ const dp=dateParts(w.createdAt || w.issuedAt); return [formatFullDateTime(w.createdAt || w.issuedAt), dp.date, dp.time, dp.month, dp.year, w.vehicle || '-', w.vehicleType || '-', w.wheelType || '-', n(w.quantity||1), w.reason || '-', w.status || '-', formatFullDateTime(w.issuedAt), w.issuedBy || '-']; });
    ...
    if(effectiveReport==='mechanics') return data.ops.map(g=>{
      ...
      return [formatFullDateTime(g.start || g.createdAt), dp.date, dp.time, dp.month, dp.year, g.vehicle, g.id, g.status, g.mechanic, g.type || g.processType || '', g.mechanicHours?.reduce?.((s,m)=>s+n(m.hours),0) || g.labor || '0', g.notes || ''];
    });
    return data.ass.map(a=>{
  ...
  async function exportRows(format){
    ...
    const summaryRows = (() => {
      const base = [['Rows Returned', tableRows.length]];
      if(effectiveReport === 'fuel') return [...base, ['Fuel Litres', data.fuel.reduce((s,f)=>s+n(f.fuelLitres),0)]];
      if(effectiveReport === 'vehicle-out') return [...base, ['Vehicle Out Count', data.out.length], ['Total Actual Time Out', dur(data.out.reduce((s,o)=>s+secs(o.startDateTime,o.endDateTime),0))]];
      if(effectiveReport === 'parts' || effectiveReport === 'low-stock') return [...base, ['Parts Issued Rows', data.issuedParts.length], ['Low Stock Count', analytics.lowStock.length], ...(isAdmin ? [['Parts Cost Total', data.partsForCost.reduce((s,p)=>s+partsCostTotal(p),0)], ['Parts Charged Total', data.partsForCost.reduce((s,p)=>s+partsChargedTotal(p),0)], ['Margin Total', data.partsForCost.reduce((s,p)=>s+partMargin(p),0)]] : [])];
      if(effectiveReport === 'mechanics') return [...base, ['Garage Visits', data.ops.length]];
      if(isAdmin) return [...base, ['Fuel Litres', data.fuel.reduce((s,f)=>s+n(f.fuelLitres),0)], ['Vehicle Out Count', data.out.length], ['Garage Visits', data.ass.length], ['Parts Issued Rows', data.issuedParts.length], ['Vehicle Cost Total', vehicleCostRows.reduce((s,r)=>s+r.totalCost,0)], ['Vehicle Charged Total', vehicleCostRows.reduce((s,r)=>s+r.totalCharged,0)], ['Vehicle Margin Total', vehicleCostRows.reduce((s,r)=>s+r.totalMargin,0)]];
      return base;
    })();
  ...
  function exportChartXlsx(name, rows, headers=['Label','Value']){
    const dataRows = rows || [];
    const dynamicHeaders = dataRows.length
      ? Array.from(dataRows.reduce((set, row) => { Object.keys(row || {}).forEach(k => set.add(k)); return set; }, new Set()))
      : headers;
    return exportProfessionalXlsx({
      ...
      rows:dataRows.map(r=>dynamicHeaders.map(h=>r?.[h] ?? '')),
      summaryRows:[['Rows Exported',dataRows.length]],
      currentUser,
      sourcePage:'Reports Chart / Analytics',
    });
  }

  function exportInventoryDetailXlsx(name, rows){
    const detailRows = (rows || []).map(item => cleanInventoryItem(item));
    const headers = ['SKU','Part','Category','Barcode','Current Stock','Reorder Level','Cost Price','Selling Price','Supplier','Supplier Email','Location','Status'];
    return exportProfessionalXlsx({
      fileName:`valle-${name}`,
      title:`${name.replaceAll('-', ' ').toUpperCase()} Inventory Detail`,
      reportType:name,
      filters,
      headers,
      rows:detailRows.map(item => [item.sku, item.name, item.category, item.barcode, item.stock, item.reorderLevel, item.costPrice, item.sellingPrice, item.supplierName, item.supplierEmail, item.location, item.stock <= 0 ? 'ZERO STOCK' : item.stock <= item.reorderLevel ? 'LOW STOCK' : 'OK']),
      summaryRows:[['Rows Exported', detailRows.length], ['Total Current Stock', detailRows.reduce((sum,item)=>sum+n(item.stock),0)], ['Total Reorder Level', detailRows.reduce((sum,item)=>sum+n(item.reorderLevel),0)]],
      currentUser,
      sourcePage:'Store Keeper Reports',
    });
  }
  ...
  return <div className="page reports-page">
    ...
    <div className="metrics-grid">
      {(isAdmin||isFuel) && <>
        <Kpi label="Fuel Litres" value={`${data.fuel.reduce((s,f)=>s+n(f.fuelLitres),0).toFixed(1)} L`} note={`${data.fuel.length} fuel records`} onClick={()=>setFilters({...filters,reportType:'fuel'})}/>
        <Kpi label="Petrol Used" value={`${data.fuel.filter(f=>String(f.fuelType||'').toUpperCase().includes('PETROL')).reduce((s,f)=>s+n(f.fuelLitres),0).toFixed(1)} L`} note="Petrol by vehicle category below" onClick={()=>setFilters({...filters,reportType:'fuel'})}/>
        <Kpi label="Diesel Used" value={`${data.fuel.filter(f=>String(f.fuelType||'').toUpperCase().includes('DIESEL')).reduce((s,f)=>s+n(f.fuelLitres),0).toFixed(1)} L`} note="Diesel by vehicle category below" onClick={()=>setFilters({...filters,reportType:'fuel'})}/>
        <Kpi label="Fuel Categories" value={analytics.fuelByVehicleType.length} note="Quad, buggy, tool, jeep etc." onClick={()=>setFilters({...filters,reportType:'fuel'})}/>
      </>}
      ...
      {(isAdmin||isStore) && <>
        <Kpi label="Parts Issued Total" value={data.allStoreParts.length} note="Mechanic + distribution/sell combined" onClick={()=>setFilters({...filters,reportType:'parts'})}/>
        <Kpi label="To Mechanics" value={data.issuedParts.length} note="Assessment / garage tickets" onClick={()=>setFilters({...filters,reportType:'parts'})}/>
        <Kpi label="To Others" value={data.distributionParts.length} note="Distribution / sell section" onClick={()=>setFilters({...filters,reportType:'parts'})}/>
        <Kpi label="Parts Qty Deducted" value={data.allStoreParts.reduce((s,p)=>s+n(p.qty||p.quantity||1),0)} note="All store deductions tally together" onClick={()=>setFilters({...filters,reportType:'parts'})}/>
      </>}
      {(isAdmin||isStore) && <>
        <Kpi label="Wheel Issued" value={data.wheel.filter(w=>String(w.status||'').toUpperCase()==='ISSUED').reduce((s,w)=>s+n(w.quantity||1),0)} note="Wheel quantity issued in selected period" onClick={()=>setFilters({...filters,reportType:'wheel'})}/>
        <Kpi label="Wheel Pending" value={data.wheel.filter(w=>String(w.status||'').toUpperCase()==='REQUESTED').length} note="Mechanic wheel requests awaiting store" onClick={()=>setFilters({...filters,reportType:'wheel'})}/>
      </>}
    ...
    <div className="analytics-grid">
      {(isAdmin||isFuel) && <>
        <ChartCard title="Fuel Consumption" subtitle="Litres filtered by selected period"><ResponsiveContainer width="100%" height={280} minWidth={260} minHeight={220}><AreaChart data={analytics.fuelByPeriod}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="label" label={{ value: "X Axis: Date", position: "insideBottom", offset: -3 }}/><YAxis label={{ value: "Y Axis: Fuel Litres", angle: -90, position: "insideLeft" }}/><Tooltip content={<ChartTooltip valueLabel="Fuel" suffix=" L" xAxisName="Date" yAxisName="Fuel Litres" />}/><Area dataKey="value" fill="#24f66f" stroke="#24f66f" onClick={(d)=>setSelected(d?.payload?.label)}/></AreaChart></ResponsiveContainer></ChartCard>
        <ChartCard title="Fuel Distribution by Vehicle Category" subtitle="Quad, buggy, heavy duty, tool and other category usage."><BarList rows={analytics.fuelByVehicleType.map(x=>({...x,displayValue:`${Number(x.value||0).toFixed(1)} L`}))}/></ChartCard>
        <ChartCard title="Petrol by Category" subtitle="Petrol litres by vehicle/tool category."><BarList rows={analytics.petrolByVehicleType.map(x=>({...x,displayValue:`${Number(x.value||0).toFixed(1)} L`}))}/></ChartCard>
        <ChartCard title="Diesel by Category" subtitle="Diesel litres by vehicle/tool category."><BarList rows={analytics.dieselByVehicleType.map(x=>({...x,displayValue:`${Number(x.value||0).toFixed(1)} L`}))}/></ChartCard>
        <ChartCard title="Fuel by Vehicle Plate" subtitle="Click a bar to filter by vehicle"><ResponsiveContainer width="100%" height={280} minWidth={260} minHeight={220}><BarChart data={analytics.fuelByVehicle}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="label" label={{ value: "X Axis: Vehicle Plate", position: "insideBottom", offset: -3 }}/><YAxis label={{ value: "Y Axis: Fuel Litres", angle: -90, position: "insideLeft" }}/><Tooltip content={<ChartTooltip valueLabel="Fuel" suffix=" L" xAxisName="Vehicle Plate" yAxisName="Fuel Litres" />}/><Bar dataKey="value" fill="#6f3cff" onClick={(d)=>setSelected(d?.label)}/></BarChart></ResponsiveContainer></ChartCard>
      </>}
      {(isAdmin||isVehicle) && <>
        <ChartCard title="Vehicle Out Frequency" subtitle="Each dot is a vehicle. Hover shows plate, ticket count and every out time for the selected period.">{analytics.outVehicleDots.length?<ResponsiveContainer width="100%" height={280} minWidth={260} minHeight={220}><ScatterChart data={analytics.outVehicleDots}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="x" type="number" name="Vehicle" tickFormatter={(v)=>analytics.outVehicleDots[v-1]?.plate || v} label={{ value: "X Axis: Vehicle Plate", position: "insideBottom", offset: -3 }}/><YAxis dataKey="y" type="number" name="Tickets" allowDecimals={false} label={{ value: "Y Axis: Ticket Count", angle: -90, position: "insideLeft" }}/><Tooltip content={<ChartTooltip valueLabel="Trips" xAxisName="Vehicle Plate" yAxisName="Ticket Count" />}/><Scatter dataKey="y" fill="#ff315f" /></ScatterChart></ResponsiveContainer>:<Empty/>}</ChartCard>
      ...
      {isAdmin && <>
        <ChartCard title="Repaired Vehicles" subtitle="Garage work jobs filtered by selected period" onExport={()=>exportChartXlsx('repaired-vehicles', analytics.repairsByPeriod)}><ResponsiveContainer width="100%" height={280} minWidth={260} minHeight={220}><BarChart data={analytics.repairsByPeriod}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="label" label={{ value: "X Axis: Date", position: "insideBottom", offset: -3 }}/><YAxis label={{ value: "Y Axis: Repair Count", angle: -90, position: "insideLeft" }}/><Tooltip content={<ChartTooltip valueLabel="Repairs" xAxisName="Date" yAxisName="Repair Count" />}/><Bar dataKey="value" fill="#2bb7ff"/></BarChart></ResponsiveContainer></ChartCard>
        <ChartCard title="Repair Cost Per Month" subtitle="Parts cost only, no duplicate assessment/work total"><ResponsiveContainer width="100%" height={280} minWidth={260} minHeight={220}><AreaChart data={analytics.repairCostByMonth}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="label" label={{ value: "X Axis: Month", position: "insideBottom", offset: -3 }}/><YAxis label={{ value: "Y Axis: Parts Cost (MUR)", angle: -90, position: "insideLeft" }}/><Tooltip content={<ChartTooltip valueLabel="Repair Cost" xAxisName="Month" yAxisName="Parts Cost" />}/><Area dataKey="value" fill="#ffd84d" stroke="#ff315f"/></AreaChart></ResponsiveContainer></ChartCard>
        <ChartCard title="Mechanic Assignment Board" subtitle="Which mechanic is assigned to which ticket and vehicle plate."><BarList rows={analytics.mechanicAssignments.map(x=>({label:x.label,value:x.value,displayValue:x.status || 'Assigned'}))} onSelect={setSelected}/></ChartCard>
        <ChartCard title="Garage Visits" subtitle="One assessment = one garage visit" onExport={()=>exportChartXlsx('garage-visits', analytics.garageVisitsByPeriod)}><ResponsiveContainer width="100%" height={280} minWidth={260} minHeight={220}><LineChart data={analytics.garageVisitsByPeriod}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="label" label={{ value: "X Axis: Date", position: "insideBottom", offset: -3 }}/><YAxis label={{ value: "Y Axis: Visit Count", angle: -90, position: "insideLeft" }}/><Tooltip content={<ChartTooltip valueLabel="Garage Visits" xAxisName="Date" yAxisName="Visit Count" />}/><Line dataKey="value" stroke="#24f66f" strokeWidth={3}/></LineChart></ResponsiveContainer></ChartCard>
      </>}
      {isAdmin && <>
        <ChartCard title="Vehicle Cost Trend" subtitle="Cost per ticket/garage visit. One assessment + linked garage repair is counted once." onExport={()=>exportChartXlsx('vehicle-cost-trend', analytics.vehicleCostByPeriod)}><ResponsiveContainer width="100%" height={280} minWidth={260} minHeight={220}><AreaChart data={analytics.vehicleCostByPeriod}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="label" label={{ value: "X Axis: Month", position: "insideBottom", offset: -3 }}/><YAxis label={{ value: "Y Axis: Vehicle Cost (MUR)", angle: -90, position: "insideLeft" }}/><Tooltip content={<ChartTooltip valueLabel="Vehicle Cost" xAxisName="Month" yAxisName="Cost" />}/><Area dataKey="value" fill="#6f3cff" stroke="#2b0048"/></AreaChart></ResponsiveContainer></ChartCard>
      ...
      {(isAdmin||isStore) && <>
        <ChartCard title="Parts Issued - Mechanics + Distribution" subtitle="Combined store deductions from assessment/garage and Parts Distribution/Sell." onExport={()=>exportChartXlsx('parts-issued', analytics.partsIssued)}><ResponsiveContainer width="100%" height={280} minWidth={260} minHeight={220}><PieChart><Pie data={analytics.partsIssued} dataKey="value" nameKey="label" innerRadius={55} outerRadius={90} onClick={(d)=>setSelected(d?.label)}>{analytics.partsIssued.map((_,i)=><Cell key={i} fill={COLORS[i%COLORS.length]}/>)}</Pie><Tooltip content={<ChartTooltip valueLabel="Quantity" xAxisName="Part" yAxisName="Quantity" />}/><Legend/></PieChart></ResponsiveContainer></ChartCard>
        <ChartCard title="Parts Destination Split" subtitle="Where parts were given: mechanics or other receivers."><BarList rows={analytics.partsByDestination.map(x=>({...x,displayValue:`${x.value} qty`}))}/></ChartCard>
        <ChartCard title="Wheel Issued by Type" subtitle="Wheel report by wheel type, size and usage from mechanic requests."><BarList rows={analytics.wheelByType.map(x=>({...x,displayValue:`${x.value} wheel(s)`}))}/></ChartCard>
        <ChartCard title="Wheel Issued by Vehicle Type" subtitle="Quad, buggy and heavy-duty wheel distribution."><BarList rows={analytics.wheelByVehicleType.map(x=>({...x,displayValue:`${x.value} wheel(s)`}))}/></ChartCard>
        <ChartCard title="Stock Risk Overview" subtitle="Live DB count by stock risk level." onExport={()=>exportChartXlsx('stock-risk-overview', stockRiskData)}><ResponsiveContainer width="100%" height={280} minWidth={260} minHeight={220}><PieChart><Pie data={stockRiskData} dataKey="value" nameKey="label" innerRadius={58} outerRadius={92} label={({label,value})=>`${label}: ${value}`}>{stockRiskData.map((_,i)=><Cell key={i} fill={COLORS[i%COLORS.length]}/>)}</Pie><Tooltip/><Legend/></PieChart></ResponsiveContainer></ChartCard>
        <ChartCard title="Parts by Category" subtitle="Category distribution from InventoryItem table." onExport={()=>exportChartXlsx('parts-by-category', categoryChartData)}><ResponsiveContainer width="100%" height={280} minWidth={260} minHeight={220}><BarChart data={categoryChartData} layout="vertical" margin={{left:80,right:24,top:8,bottom:8}}><CartesianGrid strokeDasharray="3 3"/><XAxis type="number" allowDecimals={false}/><YAxis type="category" dataKey="label" width={120} tick={{fontSize:11}}/><Tooltip content={<ChartTooltip valueLabel="Parts" xAxisName="Category" yAxisName="Parts Count"/>}/><Legend/><Bar dataKey="value" name="Parts count" fill="#6f3cff" radius={[0,8,8,0]}/></BarChart></ResponsiveContainer></ChartCard>
        <StockDetailPanel title="All Low Stock Items" subtitle="All items at or below reorder level. Export includes full stock details." rows={inventoryLowStock} onExport={()=>exportInventoryDetailXlsx('all-low-stock-items', inventoryLowStock)} />
        <StockDetailPanel title="Zero Stock Items" subtitle="All items where current stock is zero. Export includes reorder level and location." rows={inventoryClean.filter((item)=>item.stock<=0)} onExport={()=>exportInventoryDetailXlsx('zero-stock-items', inventoryClean.filter((item)=>item.stock<=0))} />
        <ChartCard title="Stock Value by Category" subtitle="Inventory stock value calculated from current stock × cost price." onExport={()=>exportChartXlsx('stock-value-by-category', stockValueByCategoryData)}>{stockValueByCategoryData.length ? <ResponsiveContainer width="100%" height={280} minWidth={260} minHeight={220}><BarChart data={stockValueByCategoryData} layout="vertical" margin={{left:80,right:24,top:8,bottom:8}}><CartesianGrid strokeDasharray="3 3"/><XAxis type="number"/><YAxis type="category" dataKey="label" width={120} tick={{fontSize:11}}/><Tooltip content={<ChartTooltip valueLabel="Stock Value" xAxisName="Category" yAxisName="Value (MUR)"/>}/><Legend/><Bar dataKey="value" name="Stock value" fill="#24f66f" radius={[0,8,8,0]}/></BarChart></ResponsiveContainer> : <Empty/>}</ChartCard>
      </>}
    </div>
