...
export default function Dashboard() {
  ...
  const myOpened = opened.filter(
    (a) => currentUser.role !== "mechanic" || a.mechanic === currentUser.name,
  );
  const ongoing = garageOps.filter(
    (g) =>
      !["Completed", "Delivered", "Cancelled", "Canceled"].includes(g.status),
  );
  const myOngoing = ongoing.filter(
    (g) =>
      currentUser.role !== "mechanic" ||
      g.mechanic === currentUser.name ||
      g.mechanic === "Workshop Team",
  );
  const adminRequests = transactions.filter(
    (t) =>
  ...
  const stats = [
    ...
    ["In Garage", ongoing.length, "Repair / service queue", "/garage"],
    ["Low Stock", lowStock.length, "Needs reorder", "/inventory"],
    ["Assessments", assessments.length, "Diagnosis records", "/assessments"],
    ["Garage Tickets", ongoing.length, "Active garage jobs", "/garage"],
    [
      "Guest Tickets",
      guestTickets?.filter((t) => t.status === "Pending").length || 0,
      "Drop-off waiting mechanic",
      "/guest-pending",
    ],
  ];
  return (
    <div className="page">
      <PageHeader
        ...
        subtitle="A simple view of vehicles, parts, admin requests and work that needs attention."
      />
      ...
      <div className="dashboard-grid">
        <Card>
          ...
          <p>Click a vehicle to view full details and history.</p>
          <div className="mini-table">
            {vehicles.slice(0, 15).map((v) => (
              <button key={v.id} onClick={() => navigate("/vehicles")}>
                ...
                <span>
                  {v.checkInDateTime?.replace("T", " ") || `${v.hours} hrs`}
                </span>
              </button>
            ))}
          </div>
          {vehicles.length > 15 && (
            <button className="open-btn" onClick={() => navigate("/vehicles")}>
              View remaining {vehicles.length - 15} vehicles
            </button>
          )}
        </Card>
        <Card>
      ...
      {currentUser.role === "mechanic" && (
        <div className="two-grid">
          <Card>
            ...
            <p>Assessments that are opened/in progress and may need action.</p>
            {myOpened.map((a) => (
          ...
          <Card>
            ...
            <p>
              Garage jobs and build requests assigned to you or the workshop
              team.
            </p>
            {myOngoing.map((g) => (
          ...
          {(guestTickets || [])
            ...
            .map((t) => (
              <div className="list-row" key={t.id}>
                <b>{t.id}</b>
                <span>
                  {t.plate} • {t.name || t.deliveryPersonName}
                </span>
      ...
      {currentUser.role === "mechanic" && (
        <Card className="section-small">
          <h2>Admin Build / Repair Requests</h2>
          {adminRequests.map((t) => (
            <div className="list-row" key={t.id}>
              <b>{t.id}</b>
              <span>
                {t.item} • Expected {t.expectedDeliveryDate || "TBD"}
              </span>
      ...
      {currentUser.role === "store" && (
        <div className="two-grid">
          <Card>
            ...
            <p>
              Open tickets from mechanics. View required parts and issue stock.
            </p>
            {opened.map((a) => (
              <div className="list-row" key={a.id}>
                <b>{a.id}</b>
