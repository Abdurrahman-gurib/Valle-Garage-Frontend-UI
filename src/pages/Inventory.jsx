...
export default function Inventory(){
  ...
  return <div className="page inventory-page">
    ...
    <div className="store-analytics-grid section-small">
      <Card className="chart-card pro-chart-card"><h2>Stock Risk Overview</h2><p className="muted">Accurate live DB count by stock risk level.</p><div className="chart-box">{stockRiskData.some(x=>x.value>0) ? <ResponsiveContainer width="100%" height={280} minWidth={260} minHeight={220}><PieChart><Pie data={stockRiskData} dataKey="value" nameKey="label" innerRadius={58} outerRadius={92} label={({label,value})=>`${label}: ${value}`}>{stockRiskData.map((_,i)=><Cell key={i} fill={['#24f66f','#ff8b00','#ff315f'][i%3]}/>)}</Pie><Tooltip/><Legend/></PieChart></ResponsiveContainer> : <div className="empty-chart">No inventory data.</div>}</div></Card>
      <Card className="chart-card pro-chart-card"><h2>Parts by Category</h2><p className="muted">Category distribution from InventoryItem table.</p><div className="chart-box">{categoryChartData.length ? <ResponsiveContainer width="100%" height={280} minWidth={260} minHeight={220}><BarChart data={categoryChartData} layout="vertical" margin={{left:80,right:20}}><CartesianGrid strokeDasharray="3 3"/><XAxis type="number" allowDecimals={false}/><YAxis type="category" dataKey="name" width={110}/><Tooltip/><Legend/><Bar dataKey="value" name="Parts count" fill="#6f3cff"/></BarChart></ResponsiveContainer> : <div className="empty-chart">No category data.</div>}</div></Card>
