...
export default function ExternalPartsIssue(){
  ...
  return <div className="page external-parts-page">
    ...
    <div className="store-analytics-grid section-small">
      <Card className="chart-card pro-chart-card"><h2>Parts Issued by Receiver Type</h2><p className="muted">Quantity issued outside garage workflow.</p><div className="chart-box"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={byType} dataKey="value" nameKey="label" innerRadius={55} outerRadius={92} label>{byType.map((_,i)=><Cell key={i} fill={COLORS[i%COLORS.length]}/>)}</Pie><Tooltip/><Legend/></PieChart></ResponsiveContainer></div></Card>
      <Card className="chart-card pro-chart-card"><h2>Top Parts Issued</h2><p className="muted">Most issued parts by quantity.</p><div className="chart-box"><ResponsiveContainer width="100%" height="100%"><BarChart data={byPart} layout="vertical" margin={{left:90,right:20}}><CartesianGrid strokeDasharray="3 3"/><XAxis type="number" allowDecimals={false}/><YAxis type="category" dataKey="label" width={140} tick={{fontSize:10}}/><Tooltip/><Bar dataKey="value" fill="#6f3cff"/></BarChart></ResponsiveContainer></div></Card>
      <Card className="chart-card pro-chart-card"><h2>Value by Receiver</h2><p className="muted">Selling value issued to each receiver/name/company.</p><div className="chart-box"><ResponsiveContainer width="100%" height="100%"><BarChart data={byReceiver} layout="vertical" margin={{left:90,right:20}}><CartesianGrid strokeDasharray="3 3"/><XAxis type="number"/><YAxis type="category" dataKey="label" width={140} tick={{fontSize:10}}/><Tooltip formatter={(v)=>money(v)}/><Bar dataKey="value" fill="#24f66f"/></BarChart></ResponsiveContainer></div></Card>
      <Card className="chart-card pro-chart-card"><h2>Issue Trend</h2><p className="muted">Daily non-garage parts quantity.</p><div className="chart-box"><ResponsiveContainer width="100%" height="100%"><LineChart data={byDay}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="label"/><YAxis allowDecimals={false}/><Tooltip/><Line dataKey="value" stroke="#6f3cff" strokeWidth={3}/></LineChart></ResponsiveContainer></div></Card>
    </div>
