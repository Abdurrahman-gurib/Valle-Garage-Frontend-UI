...
export default function GuestPending() {
  ...
  function afterVehicleSaved(ticket, savedVehicle) {
    const vehicle =
      savedVehicle ||
      findVehicleByPlate(ticket.plate) ||
      {
        id: `TEMP-${ticket.plate}`,
        plate: ticket.plate,
      };
  ...
  return (
    <div className="page guest-pending-page">
      ...
      {pendingTickets.length === 0 && (
        <Card>
          <h2>No guest tickets pending</h2>
          <p>
            When a guest submits a garage drop-off ticket, it will appear here.
          </p>
        </Card>
      )}
      ...
      >
        {guestTickets.map((t) => (
          <tr key={t.id}>
            ...
            <td>
              <Badge tone={t.status === 'Pending' ? 'warning' : 'success'}>
                {t.status}
              </Badge>
            </td>
      ...
      {modal?.type === 'view' && (
        <Modal
          title={`View Guest Ticket - ${modal.ticket.id}`}
      ...
      {modal?.type === 'take' && (
        <Modal
          title={`Take Guest Ticket - ${modal.ticket.id}`}
          onClose={() => setModal(null)}
          wide
        >
          <div className="notice">
            Taking the ticket opens the normal Add Vehicle form. After saving,
            the ticket is marked as taken. Then go to Assessments to create/start
            the assessment normally.
          </div>

          <VehicleForm
            initialVehicle={vehicleDraft(modal.ticket)}
            onDone={(savedVehicle) =>
              afterVehicleSaved(modal.ticket, savedVehicle)
            }
          />
        </Modal>
      )}
      ...
      {modal?.type === 'done' && (
        <Modal title="Guest Ticket Taken" onClose={() => setModal(null)} wide>
          <Card>
            <h2>Vehicle saved and ticket taken</h2>

            <p>
              <b>Plate:</b> {modal.vehicle?.plate || modal.ticket.plate}
            </p>

            <p>
              The ticket is no longer pending. Continue the original flow by
              opening <b>Assessments</b> and creating the assessment for this
              vehicle.
            </p>
          </Card>
    ...
    <div className="detail-stack">
      <div className="vehicle-hero-card guest-ticket-hero-card">
        ...
        <div>
          ...
          <p>
            <b>Model:</b> {ticket.model || ticket.type} • <b>VIN:</b>{' '}
            {ticket.vin || 'Not provided'}
          </p>

          <p>
            <b>Delivered by:</b>{' '}
            {ticket.name || ticket.deliveryPersonName} • <b>Contact:</b>{' '}
            {ticket.contactNumber || '-'}
          </p>

          <p>
            <b>Date/Time:</b> {ticket.createdAt?.replace('T', ' ')}
          </p>

          <p>
            <b>Reason:</b> {ticket.notes || '-'}
          </p>
        </div>
      </div>

      <div className="notice">
        View only does not assign this ticket. Click Take to continue with Add
        Vehicle.
      </div>

      <div className="form-actions">
        <Button onClick={onTake}>Take Ticket</Button>
      </div>
    </div>
  );
}