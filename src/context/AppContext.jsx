...
const roleAccess = {
  admin: ['dashboard', 'vehicles', 'assessments', 'inventory', 'garage', 'fuel', 'vehicle-out', 'transactions', 'reports', 'settings', 'notifications'],
...
function vehiclePayload(v) { return { plateNumber: v.plate, vin: v.vin || undefined, vehicleType: v.type === 'Other' ? v.customType || 'Other' : v.type, ownership: v.ownership === 'External' ? (v.sourceTransactionId ? 'CUSTOMER_ORDER' : 'EXTERNAL') : 'INTERNAL', ownerName: v.owner || undefined, companyName: v.companyName || undefined, deliveryPersonName: v.deliveryPersonName || undefined, contactNumber: v.contactNumber || undefined, email: v.email || undefined, manufacturer: v.manufacturer || 'CFMOTO', status: statusToApi[v.status] || 'ACTIVE', currentHourMeter: Number(v.hours || 0), checkInDateTime: v.checkInDateTime || undefined, expectedDeliveryDate: v.expectedDeliveryDate || undefined, notes: [v.notes, v.model ? `Model: ${v.model}` : '', v.cc ? `CC: ${v.cc}` : ''].filter(Boolean).join(' | '), transactionId: v.sourceTransactionId || undefined }; }
