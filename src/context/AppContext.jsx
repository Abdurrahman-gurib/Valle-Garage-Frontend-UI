...
const roleAccess = {
  admin: ['dashboard', 'vehicles', 'assessments', 'inventory', 'external-parts', 'wheel', 'garage', 'fuel', 'vehicle-out', 'reports', 'support', 'audit-trail', 'settings', 'notifications'],
  mechanic: ['dashboard', 'vehicles', 'assessments', 'garage', 'wheel', 'guest-pending', 'support', 'notifications'],
  store: ['dashboard', 'vehicles', 'assessments', 'inventory', 'external-parts', 'wheel', 'reports', 'support', 'notifications'],
...
export const nowLocalInput = () => formatInput();
export const todayLocal = () => todayInput();
...
function normalizeAssessment(a) { const v = a.vehicle ? normalizeVehicle(a.vehicle) : null; const partRows = arr(a.partIssues && a.partIssues.length ? a.partIssues : (a.requiredParts || a.parts)).map(normalizeMoneyPart); return { id: a.ticketNo || a.id, dbId: a.id, vehicleId: a.vehicleId, vehicle: v?.plate || a.vehiclePlate || a.vehicle || '', mechanic: a.assignedMechanics?.map?.(m=>m.mechanic?.name).filter(Boolean).join(', ') || a.mechanic?.name || a.mechanic || '', issue: a.issuesDetected || a.issue || '', conclusion: a.conclusion || '', parts: partRows, partsTotalCost: a.partsTotalCost, status: apiToAssessment[a.status] || a.status || 'Opened', reopenedBy: a.reopenedBy?.name || a.reopenedBy || '', reopenReason: a.reopenReason || '', issuedPartsNote: a.issuedPartsNote || '', issuedAt: normalizeDate(a.issuedAt) || a.issuedAt || '', issuedBy: a.issuedBy?.name || a.issuedBy || '', photos: arr(a.photos), createdAt: a.createdAt ? formatDateTime(a.createdAt) : '' }; }
function normalizeGarage(g) { const v = g.vehicle ? normalizeVehicle(g.vehicle) : null; const a = g.assessment ? normalizeAssessment(g.assessment) : null; const mh = arr(g.mechanicHours).map(m=>({ mechanicId:m.mechanicId, mechanic:m.mechanic?.name || '', hours:Number(m.hours||0), notes:m.notes || '' })); const partRows = arr(g.partSales && g.partSales.length ? g.partSales : g.partsUsed).map(normalizeMoneyPart); const mechanicsText = mh.map(x=>x.mechanic).filter(Boolean).join(', ') || g.mechanic?.name || g.mechanic || ''; return { id: g.processNo || g.id, dbId: g.id, vehicleId: g.vehicleId, vehicle: v?.plate || g.vehiclePlate || g.vehicle || '', assessmentId: a?.id || g.assessmentId || '', transactionId: g.transactionId || '', type: apiToGarage[g.processType] || g.type || 'Repair', mechanic: mechanicsText, mechanicNames: mechanicsText, mechanicHours: mh, checkInDateTime: normalizeDate(g.checkInDateTime), start: normalizeDate(g.startDateTime) || g.start || '', partsSubmittedAt: normalizeDate(g.partsSubmittedAt) || a?.issuedAt || g.partsSubmittedAt || '', end: normalizeDate(g.endDateTime) || g.end || 'Pending', endDateTime: normalizeDate(g.endDateTime) || g.endDateTime || '', labor: g.laborHours ? `${g.laborHours} hrs` : g.labor || '0 hrs', laborHours: Number(g.laborHours || 0), expectedDeliveryDate: g.expectedDeliveryDate ? String(g.expectedDeliveryDate).slice(0,10) : (v?.expectedDeliveryDate || ''), status: apiToGarageStatus[g.status] || g.status || 'Pending', invoiceFile: g.invoiceAttachmentUrl || g.invoiceFile || '', workDone: g.proceduresPerformed || g.workDone || '', partsUsed: partRows, photos: arr(g.photos), createdAt: normalizeDate(g.createdAt) || g.createdAt || '' }; }
...
function normalizeFuel(f) { const v = f.vehicle ? normalizeVehicle(f.vehicle) : null; return { id:f.id, vehicleId:f.vehicleId, vehicle:v?.plate || f.vehiclePlate || f.vehicle || '', fuelType:f.fuelType || '', meterType:f.meterType || 'KM', meterReading:Number(f.meterReading || 0), fuelLitres:Number(f.fuelLitres || 0), notes:f.notes || '', fuelGivenBy:f.fuelGivenBy || '', recordedAt:normalizeDate(f.recordedAt), recordedBy:f.recordedBy?.name || '' }; }
...
export function AppProvider({ children }) {
  ...
  async function refreshAll() {
    ...
    const canLoadFuelStock = ['admin', 'fuel'].includes(role);
    const canLoadWheelRequests = ['admin', 'mechanic', 'store'].includes(role);
    const canLoadStoreData = ['admin', 'store'].includes(role);
  ...
  async function addFuelStockLog(data){
    const saved = await safe('add fuel stock refill', () => api.fuelStock.create(data), null);
    if(saved){
      if(saved.stockSummary) notify(`Fuel refill saved. Petrol reserve: ${Number(saved.stockSummary.petrolReserve||0).toFixed(2)} L, Diesel reserve: ${Number(saved.stockSummary.dieselReserve||0).toFixed(2)} L`, 'success');
      await refreshAll();
    }
    return saved;
  }

  async function addWheelRequest(data){
    const vehicle=vehicles.find(v=>v.id===data.vehicleId || v.dbId===data.vehicleId);
    const saved = await safe('create wheel request', () => api.wheels.create({ ...data, vehicleId: vehicle?.dbId || data.vehicleId, vehicleType: vehicle?.type || data.vehicleType }), null);
    if(saved){ setWheelRequests(prev=>[normalizeWheelRequest(saved), ...prev]); await refreshAll(); }
    return saved;
  }
  async function updateWheelRequest(id, data){
    const saved = await safe('update wheel request', () => api.wheels.update(id, data), null);
    if(saved){ setWheelRequests(prev=>prev.map(w=>w.id===id ? normalizeWheelRequest(saved) : w)); await refreshAll(); }
    return saved;
  }
