...
const roleAccess = {
  admin: ['dashboard', 'vehicles', 'assessments', 'inventory', 'garage', 'fuel', 'vehicle-out', 'transactions', 'reports', 'audit-trail', 'settings', 'notifications'],
  ...
  fuel: ['fuel', 'reports', 'notifications'],
  vehicle_manager: ['vehicle-out', 'reports', 'notifications'],
  guest: ['guest']
};
...
function normalizeAssessment(a) { const v = a.vehicle ? normalizeVehicle(a.vehicle) : null; const partRows = arr(a.requiredParts || a.parts || a.partIssues).map(p => ({ partId: p.partId || p.inventoryItemId || p.id || 'manual', sku: p.sku || '', name: p.name || p.partName || p.part || p.inventoryItem?.name || '', qty: Number(p.qty || p.quantity || 1), sellingPrice: Number(p.sellingPrice || p.inventoryItem?.sellingPrice || 0), lineTotal: Number(p.lineTotal || Number(p.qty || p.quantity || 1) * Number(p.sellingPrice || p.inventoryItem?.sellingPrice || 0)) })); return { id: a.ticketNo || a.id, dbId: a.id, vehicleId: a.vehicleId, vehicle: v?.plate || a.vehiclePlate || a.vehicle || '', mechanic: a.assignedMechanics?.map?.(m=>m.mechanic?.name).filter(Boolean).join(', ') || a.mechanic?.name || a.mechanic || '', issue: a.issuesDetected || a.issue || '', conclusion: a.conclusion || '', parts: partRows, partsTotalCost: a.partsTotalCost, status: apiToAssessment[a.status] || a.status || 'Opened', reopenedBy: a.reopenedBy?.name || a.reopenedBy || '', reopenReason: a.reopenReason || '', issuedPartsNote: a.issuedPartsNote || '', photos: arr(a.photos), createdAt: a.createdAt ? new Date(a.createdAt).toLocaleString() : '' }; }
function normalizeGarage(g) { const v = g.vehicle ? normalizeVehicle(g.vehicle) : null; const a = g.assessment ? normalizeAssessment(g.assessment) : null; const mh = arr(g.mechanicHours).map(m=>({ mechanicId:m.mechanicId, mechanic:m.mechanic?.name || '', hours:Number(m.hours||0) })); return { id: g.processNo || g.id, dbId: g.id, vehicleId: g.vehicleId, vehicle: v?.plate || g.vehicle || '', assessmentId: a?.id || g.assessmentId || '', transactionId: g.transactionId || '', type: apiToGarage[g.processType] || g.type || 'Repair', mechanic: mh.map(x=>x.mechanic).filter(Boolean).join(', ') || g.mechanic?.name || g.mechanic || '', mechanicHours: mh, checkInDateTime: normalizeDate(g.checkInDateTime), start: normalizeDate(g.startDateTime) || g.start || '', end: normalizeDate(g.endDateTime) || g.end || 'Pending', labor: g.laborHours ? `${g.laborHours} hrs` : g.labor || '0 hrs', expectedDeliveryDate: g.expectedDeliveryDate ? String(g.expectedDeliveryDate).slice(0,10) : '', status: apiToGarageStatus[g.status] || g.status || 'Pending', paymentStatus: g.paymentDone ? 'Paid' : g.paymentStatus || 'Pending', invoiceFile: g.invoiceAttachmentUrl || g.invoiceFile || '', workDone: g.proceduresPerformed || g.workDone || '', partsUsed: arr(g.partsUsed).map(p => ({ partId:p.partId || p.id || 'manual', name: p.name || p.partName || p.part || '', qty: Number(p.qty || p.quantity || 1), sellingPrice:Number(p.sellingPrice || p.price || p.unitPrice || 0) })), photos: arr(g.photos) }; }
...
function normalizeVehicleOut(o) { const v = o.vehicle ? normalizeVehicle(o.vehicle) : null; return { id:o.id, vehicleId:o.vehicleId, vehicle:v?.plate || o.vehiclePlate || o.vehicle || '', activityType:o.activityType || 'Activity', destination:o.destination || '', driverName:o.driverName || '', invoiceNumber:o.invoiceNumber || '', guideName:o.guideName || '', quadActivity:o.quadActivity || '', tripDuration:o.tripDuration || '', startDateTime:normalizeDate(o.startDateTime), endDateTime:normalizeDate(o.endDateTime), notes:o.notes || '', recordedBy:o.recordedBy?.name || '' }; }
...
function garagePayload(data) { return { vehicleId: data.vehicleId, assessmentId: data.assessmentId || undefined, processType: garageToApi[data.type] || 'REPAIR', status: garageStatusToApi[data.status] || 'IN_PROGRESS', proceduresPerformed: data.workDone || undefined, partsUsed: arr(data.partsUsed).map(p => ({ partId:p.partId, partName: p.name, name:p.name, quantity: Number(p.qty || 1), qty:Number(p.qty || 1), sellingPrice:Number(p.sellingPrice || p.price || p.unitPrice || 0) })), mechanicIds: data.mechanicIds || undefined, mechanicHours: data.mechanicHours || undefined, checkInDateTime: data.checkInDateTime || undefined, startDateTime: data.start || undefined, laborHours: parseFloat(String(data.labor || '0').replace(/[^0-9.]/g,'')) || 0, invoiceAttachmentUrl: data.invoiceFile || undefined, paymentDone: data.paymentStatus === 'Paid', photos: arr(data.photos) }; }
...
export function AppProvider({ children }) {
  ...
  async function addVehicle(vehicle){
    ...
    setVehicles(prev => {
      const exists = prev.find(v => v.id === savedVehicle.id || v.dbId === savedVehicle.dbId || normPlate(v.plate) === normPlate(savedVehicle.plate));
      return exists ? prev.map(v => (v.id === savedVehicle.id || v.dbId === savedVehicle.dbId || normPlate(v.plate) === normPlate(savedVehicle.plate)) ? { ...v, ...savedVehicle } : v) : [savedVehicle, ...prev];
    });
  ...
  async function createInventoryItem(data){ const local={ id:`PART-${Date.now()}`, ...data, name:data.name || data.part, stock:Number(data.stock || data.currentStock || 0), lastPrice:Number(data.sellingPrice || data.lastPrice || 0), supplier:data.supplier || data.supplierName || '' }; setInventory(prev=>[local,...prev]); await safe('create inventory item', () => api.inventory.create({ sku:local.sku, name:local.name, currentStock:local.stock, reorderLevel:Number(local.reorderLevel||0), sellingPrice:Number(local.lastPrice||0), supplierName:local.supplier, supplierEmail:local.supplierEmail, category:local.category, location:local.location }), refreshAll); return local; }
  async function updateInventoryItem(id, updates){ const item=inventory.find(i=>i.id===id || i.dbId===id); const merged={...(item||{}),...updates}; setInventory(prev=>prev.map(i=>i.id===id || i.dbId===id ? merged : i)); await safe('update inventory item', () => api.inventory.update(item?.dbId || id, { sku:merged.sku, name:merged.name || merged.part, currentStock:Number(merged.stock ?? merged.currentStock ?? 0), reorderLevel:Number(merged.reorderLevel||0), sellingPrice:Number(merged.sellingPrice ?? merged.lastPrice ?? 0), supplierName:merged.supplier || merged.supplierName, supplierEmail:merged.supplierEmail, category:merged.category, location:merged.location }), refreshAll); }
