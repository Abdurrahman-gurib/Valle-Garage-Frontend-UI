...
export function Card({ children, className = '', onClick }) {
  ...
  return <Tag onClick={onClick} className={`card ${onClick ? 'card-click' : ''} ${className}`}>{children}</Tag>;
}
export function PageHeader({ title, subtitle, action, actionLabel }) {
  return <div className="page-header"><div><p className="kicker">VALLÉ INTERNAL SYSTEM</p><h1>{title}</h1>{subtitle && <p className="subtitle">{subtitle}</p>}</div>{action && <Button onClick={action}>{actionLabel}</Button>}</div>;
}
